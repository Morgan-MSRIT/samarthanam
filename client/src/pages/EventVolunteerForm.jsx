      ...
      <div className="container-fluid">
        <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          ...
          <form onSubmit={handleSubmit} className="space-y-6 bg-accent p-8 rounded-lg shadow">
            <div>
              <label htmlFor="availableHours" className="block text-sm font-medium text-primary">
                Available Hours *
              </label>
              <input
                type="number"
            ...
            <div>
              ...
              {event?.tasks && event.tasks.length > 0 ? (
                <div className="space-y-2">
                  {event.tasks.map((task) => (
                    <div key={task._id} className="flex items-center">
                      ...
                      />
                      <label htmlFor={`task-${task._id}`} className="ml-2 block text-sm text-primary">
                        {task.name} ({new Date(task.startTime).toLocaleString()} - {new Date(task.endTime).toLocaleString()})
                        <br />
                        <span className="text-xs text-secondary">
                          Volunteers needed: {task.currentVolunteerCount}/{task.maxVolunteerNeeded}
                        </span>
                      </label>
                    </div>
                  ))}
                ...
                <p className="text-sm text-secondary">No tasks available for this event</p>
              )}
            </div>

            <div>
              <label htmlFor="additionalNotes" className="block text-sm font-medium text-primary">
                Additional Notes
              </label>
              <textarea
                id="additionalNotes"
